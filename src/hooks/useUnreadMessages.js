...
const canUseMessaging = (user) => {
    const role = user?.role || user?.rol || user?.RolFederacion || '';
    const normalized = String(role).toUpperCase();
    return (
        normalized === 'SUPERADMIN' ||
        normalized === 'FEDERACION' ||
        normalized === 'ADMIN' ||
        normalized === 'CLUB'
    );
};
...
/**
 * Contador de mensajes no leídos (solo sistema SIGDEF vía X-Client-App).
 * Polling silencioso; si falla la API, no rompe la UI.
 */
export default function useUnreadMessages(enabled = true) {
    const { user, isAuthenticated } = useAuth();
    const [unreadCount, setUnreadCount] = useState(0);

    const canUse = enabled && isAuthenticated && canUseMessaging(user);
